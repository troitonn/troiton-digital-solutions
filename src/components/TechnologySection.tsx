  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <div
          className={`grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-8 p-6 rounded-2xl transition-all duration-1000 ${
            isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'
          }`}
        >
          {technologies.map((tech, index) => (
            <TechnologyCard key={index} logo={tech.logo} />
          ))}
        </div>
      </div>
    </section>
